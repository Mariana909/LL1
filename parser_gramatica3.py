...
class Parser:
    ...
    def parse_S(self, nodo_padre):
        ...
        if self.parse_A(nodo) and self.parse_B(nodo) and self.parse_C(nodo) and self.parse_Sp(nodo):
            return True
    ...
    # S' → uno S' | ε
    def parse_Sp(self, nodo_padre):
        nodo = nodo_padre.agregar(Nodo("S'"))
        pos_respaldo = self.pos

        if self.consumir("uno"):
            nodo.agregar(Nodo("uno"))
            if self.parse_Sp(nodo):
                return True
    ...
    def parse_A(self, nodo_padre):
        ...
        if self.consumir("dos"):
            ...
            if self.parse_B(nodo) and self.parse_C(nodo):
                return True
    ...
    # B → C tres | ε
    # CONFLICTO LL(1): pred(B→C tres) = {cuatro, tres}
    #                  pred(B→ε)      = {$, cuatro, tres, uno}
    # Se resuelve con backtracking: primero intenta B→C tres,
    # si falla retrocede y produce ε.
    ...
    # C → cuatro B | ε
    # CONFLICTO LL(1): pred(C→cuatro B) = {cuatro}
    #                  pred(C→ε)        = {$, cuatro, tres, uno}
    # Se resuelve con backtracking: primero intenta C→cuatro B,
    # si falla retrocede y produce ε.
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Uso: python3 parser_gramatica3.py <archivo>")
        sys.exit(1)
